document.addEventListener('DOMContentLoaded', () => {
    fetch('http://localhost:3000/category/fetch_category') // yahan apne Node.js server ka URL daalna hai
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            data.forEach(item => {
                ...
                option.value = item.value; // ya jis key se value lena hai
                option.textContent = item.label; // ya jis key se label lena hai
...
// console.log("dddddddddddddddddddddddddddddddddddd:",fetch())
